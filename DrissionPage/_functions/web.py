...
def get_ele_txt(e):
    # Elements that do not need to be preceded by a line break
    ...
    # Add a line break element after
    ...
    def get_node_txt(ele, pre=False) -> list:
        ...
        str_list = []
        if tag in noText_list and not pre:  # The text inside the tag is not returned
            return str_list

        nodes = ele.eles('xpath:./text() | *')
        prev_ele = ''
        for el in nodes:
            if isinstance(el, str):  # Character Node
                if pre:
                    str_list.append(el)

                else:
                    if sub('[ \n\t\r]', '', el) != '':  # Characters other than carriage return and space
                        txt = el
                        if not pre:
                            txt = txt.replace('\r\n', ' ').replace('\n', ' ')
                            txt = sub(r' {2,}', ' ', txt)
                ...
                if el.tag not in nowrap_list and str_list and str_list[-1] != '\n':  # Line breaks between elements
                    str_list.append('\n')
                if el.tag in tab_list and prev_ele in tab_list:  # Table rows
                    str_list.append('\t')

                str_list.extend(get_node_txt(el, pre))
                prev_ele = el.tag

        if tag in wrap_after_list and str_list and str_list[-1] not in ('\n', True):  # Some elements need to be followed by a carriage return
            str_list.append('\n')
...
def make_absolute_link(link, baseURI=None):
    ...
    # Is a relative path, concatenated with the page URL and returned
    if not parsed['netloc']:
        return urljoin(baseURI, link) if baseURI else link

    # It is an absolute path but lacks the protocol, get the protocol from the page url and fix it
    if not parsed['scheme'] and baseURI:
        parsed['scheme'] = urlparse(baseURI).scheme
        parsed = tuple(v for v in parsed.values())
        return urlunparse(parsed)

    # Absolute path without missing protocol, return directly
    return link
...
def get_blob(page, url, as_bytes=True):
    if not url.startswith('blob'):
        raise TypeError('The link is not of blob type.')
    js = """
...
"""
    try:
        ...
        raise RuntimeError('Unable to retrieve the resource.')
    if as_bytes:
...
def get_pdf(page, path=None, name=None, kwargs=None):
    ...
    try:
        ...
        raise RuntimeError('Save failed, maybe the browser version is not supported.')
    from base64 import b64decode
    r = b64decode(r)
    if path is None and name is None:
        return r
