...
class ChromiumOptions(object):
    def __init__(self, read_file=True, ini_path=None):
        ...
        if read_file is False:
            ...
            if not ini_path.exists():
                raise ValueError(f'File does not exist: {ini_path}')
    ...
    def add_extension(self, path):
        path = Path(path)
        if not path.exists():
            raise OSError('The plugin path does not exist.')
    ...
    def set_proxy(self, proxy):
        if search(r'.*?:.*?@.*?\..*', proxy):
            print('You seem to be setting up a proxy that uses an account and password. This type of proxy is not currently supported. You can use a plug-in to meet your needs.')
        if proxy.lower().startswith('socks'):
            print('You seem to be using socks proxy, which is not supported at the moment. You can use plugins to meet your needs.')
        self._proxy = proxy
        return self.set_argument('--proxy-server', proxy)

    def set_load_mode(self, value):
        if value not in ('normal', 'eager', 'none'):
            raise ValueError("Only 'normal', 'eager', 'none' can be selected.")
    ...
    def set_paths(self, browser_path=None, local_port=None, address=None, download_path=None,
        ...
        :param address: debug browser address, example: 127.0.0.1:9222
