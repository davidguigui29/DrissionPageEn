...
class Driver(object):
    def __init__(self, tab_id, tab_type, address, owner=None):
        ...
        # self._debug = True
        # self._debug = False
        self.alert_flag = False  # Mark alert appears, skip a request and then restore
    ...
    def start(self):
        ...
        try:
            ...
            if 'Handshake status 403 Forbidden' in str(e):
                raise RuntimeError('Please upgrade the websocket-client library.')
            else:
                return
        except ConnectionRefusedError:
            raise BrowserConnectError('The browser is not open or has been closed.')
        self._recv_th.start()
