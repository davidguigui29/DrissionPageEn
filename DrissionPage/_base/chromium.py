...
class Chromium(object):
    ...
    @property
    def tabs_count(self):
        j = self._run_cdp('Target.getTargets')['targetInfos']  # Don't use get instead to avoid stuck
        return len([i for i in j if i['type'] in ('page', 'webview') and not i['url'].startswith('devtools://')])

    @property
    def tab_ids(self):
        j = self._driver.get(f'http://{self.address}/json').json()  # Don't use cdp instead, because the order is wrong
    ...
    def get_tab(self, id_or_num=None, title=None, url=None, tab_type='page'):
        ...
        if t._type != 'MixTab':
            raise RuntimeError('This tab already has a non-MixTab version. If you want to share it with multiple objects, please use Settings to set singleton_tab_obj to False.')
        return t
    ...
    def close_tabs(self, tabs_or_ids=None, others=False):
        ...
        if isinstance(tabs_or_ids, str):
            ...
            raise TypeError('The tabs_or_ids parameter can only pass in tab objects or ids.')
    ...
    def _get_tab(self, id_or_num=None, title=None, url=None, tab_type='page', mix=True, as_id=False):
        if id_or_num is not None:
            if isinstance(id_or_num, int):
                ...
                raise ValueError(f'Tab not found{id_or_num}，All Tabs：{self.tab_ids}')
    ...
    def _get_tabs(self, title=None, url=None, tab_type='page', mix=True, as_id=False):
        tabs = self._driver.get(f'http://{self.address}/json').json()  # Don't use cdp instead
        ...
        if isinstance(tab_type, str):
            ...
            raise TypeError('tab_type can only be set, list, tuple, str, None.')
...
def handle_options(addr_or_opts):
    """Set browser startup properties
    :param addr_or_opts: 'ip:port', ChromiumOptions, Driver
    :return: Returns the ChromiumOptions object
    """
    if not addr_or_opts:
        ...
        raise TypeError('Can only accept parameters in ip:port format or ChromiumOptions type.')
...
def run_browser(chromium_options):
    ...
    try:
        ...
        if not ws:
            raise BrowserConnectError('\nBrowser connection failed, please confirm whether the browser is started.')
        json = ws.json()
        ...
        raise BrowserConnectError('\nThe browser version is too old or does not support takeover.')
    except:
        raise BrowserConnectError('\nBrowser connection failed, please confirm whether the browser is started.')
