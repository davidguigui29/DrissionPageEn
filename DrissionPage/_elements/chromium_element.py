...
class ChromiumElement(DrissionElement):
    ...
    def attrs(self):
        try:
            ...
            attrs = self.owner._run_cdp('DOM.getAttributes', nodeId=self._node_id)['attributes']
            return {attrs[i]: attrs[i + 1] for i in range(0, len(attrs), 2)}
        except CDPError:  # This method cannot be called on the document root element
            return {}
    ...
    def check(self, uncheck=False, by_js=False):
        """Select or uncheck the current element
        :param uncheck: whether to uncheck
        :param by_js: whether to execute with js
        :return: None
        """
    ...
    def offset(self, locator=None, x=None, y=None, timeout=None):
        if locator and not (isinstance(locator, str) and not locator.startswith(
            ...
            raise ValueError('The locator parameter can only be in str format and does not support xpath and css formats.')
    ...
    def attr(self, attr):
        attrs = self.attrs
        if attr == 'href':  # Returns the absolute URL when getting the href attribute
            link = attrs.get('href')
    ...
    def src(self, timeout=None, base64_to_bytes=True):
        if timeout is None:
            timeout = self.timeout
        if self.tag == 'img':  # Wait for the image to load
        ...
        if not src:
            raise RuntimeError('The element has no src value or the value is empty.')
    ...
    def drag_to(self, ele_or_loc, duration=.5):
        if isinstance(ele_or_loc, ChromiumElement):
            ...
            raise TypeError('A ChromiumElement object or coordinates are required.')
...
class ShadowRoot(BaseElement):
    ...
    def parent(self, level_or_loc=1, index=1, timeout=0):
        if isinstance(level_or_loc, int):
            ...
            if loc[0] == 'css selector':
                raise ValueError('This css selector syntax is not supported, please use xpath instead.')

            loc = f'xpath:./ancestor-or-self::{loc[1].lstrip(". / ")}[{index}]'

        else:
            raise TypeError('The level_or_loc parameter can only be tuple, int or str.')
    ...
    def child(self, locator='', index=1, timeout=None):
        if not locator:
            ...
            loc = get_loc(locator, True)  # Convert locators to xpath
            if loc[0] == 'css selector':
                raise ValueError('This css selector syntax is not supported, please use xpath instead.')
            loc = loc[1].lstrip('./')
    ...
    def do_find():
        ...
        if 'exceptionDetails' in res:
            if 'The result is not a node set' in res['result']['description']:
                ...
                raise SyntaxError(f'Query statement error: \n{res}')
...
def _get_node_by_obj_id(page, obj_id, ele_only):
    """Returns the element object or text according to obj id.
    If it is text, it returns None when ele_only is used. If an error occurs, it returns False"""
...
def _get_node_by_node_id(page, node_id, ele_only):
    """Returns the element object or text according to the node id.
    If it is text, it returns None when ele_only is used. If an error occurs, it returns False"""
...
def make_js_for_find_ele_by_xpath(xpath, type_txt, node_txt):
    ...
    # Get the first element, node, or attribute
    if type_txt == '9':
        return_txt = '''
if(e.singleNodeValue==null){return null;}
    ...
    # Get all elements, nodes, or attributes in order
    elif type_txt == '7':
        for_txt = """
let a=new Array();
...
def run_js(page_or_ele, script, as_expr, timeout, args=None):
    if isinstance(page_or_ele, (ChromiumElement, ShadowRoot)):
        ...
        while perf_counter() < end_time:
            ...
            raise RuntimeError('The js runtime environment is wrong.')
    ...
    try:
        ...
        raise TimeoutError(f'Execute js timeout (wait {timeout} seconds). ')
    except ContextLostError:
        raise ContextLostError('The page has been refreshed. Please wait until the page is loaded before performing the operation.') if is_page else ElementLostError(
            'The element object originally obtained is no longer in the page.')

    if not res:  # _timeout=0 or js activates alert
        return None
    ...
    try:
        ...
        raise RuntimeError(f'\njs result parsing error\nversion: {__version__}\ncontent: {res}\njs: {script}\n'
                           f'This error may mean that there is a bug in the program.Please inform the author of the error message and the method to reproduce it. Thank you. \n'
                           f'Report website: https://gitee.com/g1879/DrissionPage/issues')
...
def _check_ele(ele, loc_data):
    """Check whether the element meets the requirements specified by loc_data
    :param ele: element object
    :param loc_data: format: {'and': bool, 'args': ['attribute name', 'matching method', 'attribute value', whether to negate]}
    :return: bool
    """
    attrs = ele.attrs
    if loc_data['and']:
