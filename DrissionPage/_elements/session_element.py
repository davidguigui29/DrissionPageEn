...
class SessionElement(DrissionElement):
    ...
    def __init__(self, ele, owner=None):
        """Initialize object
        :param ele: The wrapped HtmlElement element
        :param owner: The page object where the element is located. If it is an element generated from html text, it is None
        """
    ...
    def html(self):
        ...
        return unescape(html[:html.rfind('>') + 1])  # tostring() will also include the text nodes of the element that follows it, so you need to remove it.
    @property
    def inner_html(self):
    ...
    def attr(self, name):
        if name == 'href':  # Returns the absolute URL when getting the href attribute
            link = self.inner_ele.get('href')
            # If the link is None, js or email, return directly
            if not link or link.lower().startswith(('javascript:', 'mailto:')):
                return link
            else:  # In other cases, return the absolute URL directly
...
def make_session_ele(html_or_ele, loc=None, index=1, method=None):
    ...
    # ---------------Get the page object and lxml element object according to the passed object type---------------
    # Directly pass in HTML text
    ...
    # SessionElement
        ...
        # If css starts with >, it means looking for the direct child elements of the element. You need to use page with an absolute path to find it.
            ...
            else:  # Receive HTML text, no page
        ...
        # Get the entire page HTML and then locate the current element to find the parent element
        page = html_or_ele.owner
        xpath = html_or_ele.xpath
        # ChromiumElement, compatible with the case where the passed element is inside an iframe
    ...
    # ShadowRoot
        ...
        raise TypeError('The html_or_ele parameter can only be an element, a page object or html text.')

    # ---------------Perform a search-----------------
    try:
        if loc[0] == 'xpath':  # Use lxml built-in methods to get a list of lxml element objects
            eles = html_or_ele.xpath(loc[1])
        else:  # Get a list of element objects using CSS selector
            eles = html_or_ele.cssselect(loc[1])

        if not isinstance(eles, list):  # The result is not a list, such as a number
            return eles

        # Wrap lxml element objects into SessionElement objects and return one or all as needed
        if index is None:
        ...
        if 'Invalid expression' in str(e):
            raise SyntaxError(f'Invalid xpath statement: {loc}')
        elif 'Expected selector' in str(e):
            raise SyntaxError(f'Invalid CSS select statement: {loc}')
