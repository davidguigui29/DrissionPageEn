...
class ChromiumBase(BasePage):
    def __init__(self, browser, target_id=None):
        ...
        self._doc_got = False  # Used to mark whether the doc has been obtained between LoadEventFired and FrameStoppedLoading
    ...
    def _onFileChooserOpened(self, **kwargs):
        if self._upload_list:
            if 'backendNodeId' not in kwargs:
                raise TypeError('This input box cannot be taken over, please use the method of inputting the path of the <input> element instead.')
    ...
    def driver(self):
        if self._driver is None:
            raise RuntimeError('The browser is closed or the connection is broken.')
        return self._driver
    ...
    def _find_elements(self, locator, timeout, index=1, relative=False, raise_err=None):
        if isinstance(locator, (str, tuple)):
            ...
            raise ValueError('The locator parameter can only be of type tuple, str, or ChromiumElement.')
    ...
    def add_ele(self, html_or_info, insert_to=None, before=None):
        if isinstance(html_or_info, str):
            ...
            raise TypeError('The html_or_info parameter must be html text or tuple, the tuple format is (tag, {name: value}).')
    ...
    def _d_connect(self, to_url, times=0, interval=1, show_errmsg=False, timeout=None):
        ...
        for t in range(times + 1):
            ...
            if not ok:
                err = TimeoutError(f'Page connection timed out (waiting {timeout} seconds).')
                if t < times:
                    sleep(interval)
                    if show_errmsg:
                        print(f'重试{t + 1} {to_url}')
        ...
        if err:
            if show_errmsg:
                raise err if err is not None else ConnectionError('Connection error.')
            return False
    ...
    def _get_screenshot(self, path=None, name=None, as_bytes=None, as_base64=None,
        ...
        if as_bytes:
            if as_bytes is True:
                ...
                if as_bytes not in ('jpg', 'jpeg', 'png', 'webp'):
                    raise TypeError("Only 'jpg', 'jpeg', 'png', 'webp' can be accepted.")
        ...
        if full_page:
            ...
            if width == 0 or height == 0:
                raise RuntimeError('The page size is 0, please try to wait for the page to load.')
...
def close_privacy_dialog(page, tid):
    """Close the privacy statement pop-up window
